				...
				<div class="card">
					<div class="card-header">
						<h2>Household list<small>List of Household</small></h2>
							...
							<a href="<?php echo site_url('admin/household/export_pdf');?>" class="pull-right hvr-pulse-grow" id="btn-Preview-Image" data-toggle="tooltip" title="Export PDF">
							<i class="fa fa-2x fa-file-pdf-o" id="" name="" style="color: #cc3300; cursor: pointer; margin-right: 20px;" onclick="csv()"></i>
						</a>

						<a href="<?php echo site_url('admin/household/export_csv');?>" class="pull-right hvr-pulse-grow" id="btn-Preview-Image" data-toggle="tooltip" title="Export Excel">
		...
		<script type="text/javascript">
			$(document).ready(function()
			{
				...
				$('#ANMCode').change(function(){
					...
					$.post('<?php echo site_url('admin/Ajax/getAshaOfAnmViaAnmCode');?>/'+ANMCode, {}, function(raw){
						$('#ASHACode').html(raw);
					});
				});
			...
			function bindBootgrid()
			{
				$("#data-table-command").bootgrid('destroy').bootgrid({
					...
					ajax: true,
					url: "<?php echo site_url("admin/Ajax/getHouseHoldList");?>"	,
					post: function ()
					{
					...
					formatters: {
						...
						{
							return  "<a href=\"<?php echo site_url('admin/Household/familymembers');?>/" + row.HHSurveyGUID + "\" type=\"button\" class=\"btn btn-icon command-edit waves-effect waves-circle\" data-row-id=\"" + row.id +"\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"\" data-original-title= \"View Activities\"><span class=\"zmdi zmdi-view-list\"></span></a>"
						}
