<section id="content">
<?php foreach ($role_permission as $row) { if ($row->Controller == "Subcenter" && $row->Action == "index") { ?>
	<?php foreach ($role_permission as $row) { 
		if ($row->Controller == "Subcenter" && $row->Action == "add") { ?>
		<div class="block-header">
			<h2>Manage Subcenter</h2><br>
			<a href="<?php echo site_url("subcenter/add/")?>" class="btn btn-primary">Add New</a>
		</div>
		<?php } } ?>
			...
			<div class="content animate-panel">
				...
				<div class="card">
					<div class="card-header">
                     ...
                     <?php foreach ($role_permission as $row) { if ($row->Controller == "subcenter" && $row->Action == "Export_pdf") { ?>
						<a href="<?php echo site_url('subcenter/export_pdf');?>" class="pull-right hvr-pulse-grow" id="btn-Preview-Image" data-toggle="tooltip" title="Export PDF">
							<i class="fa fa-2x fa-file-pdf-o" id="" name="" style="color: #cc3300; cursor: pointer; margin-right: 20px;" onclick="csv()"></i>
						</a>
						<?php } } ?>
                       <?php foreach ($role_permission as $row) { if ($row->Controller == "subcenter" && $row->Action == "export_csv") { ?>
						<a href="<?php echo site_url('subcenter/export_csv');?>" class="pull-right hvr-pulse-grow" id="btn-Preview-Image" data-toggle="tooltip" title="Export Excel">
