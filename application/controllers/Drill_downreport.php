...
class Drill_downreport extends Auth_controller {
	...
	public function index()
	{
       // start permission 
		...
		(SELECT COUNT(FamilyMemberName) from tblhhfamilymember a
inner join tblhhsurvey b on a.HHSurveyGUID=b.HHSurveyGUID inner join tblusers c on a.CreatedBy=c.user_id
 where AprilAgeYear BETWEEN 15 and 49 and GenderID=2 and c.user_mode= 1)
		as NumofWomenFifteentofortyNine,

		(SELECT COUNT(FamilyMemberName) from tblhhfamilymember a
inner join tblhhsurvey b on a.HHSurveyGUID=b.HHSurveyGUID inner join tblusers c on a.CreatedBy=c.user_id where c.user_mode= 1 and AprilAgeYear BETWEEN 35 and 50  ) as AdultsAgeThirtyFiveToFifty,

		(SELECT COUNT(FamilyMemberName) from tblhhfamilymember a
inner join tblhhsurvey b on a.HHSurveyGUID=b.HHSurveyGUID inner join tblusers c on a.CreatedBy=c.user_id where c.user_mode= 1 and AprilAgeYear >60  ) as AdultsAgeMorethanSixty,

		(SELECT COUNT(FamilyMemberName) from tblhhfamilymember a
inner join tblhhsurvey b on a.HHSurveyGUID=b.HHSurveyGUID inner join tblusers c on a.CreatedBy=c.user_id where c.user_mode= 1 and AshaID!='' and GenderID=2) as totalPregnantWomen

		FROM tblhhfamilymember ";

		if ($this->loginData->user_role == 6 ) {
			$sqlQuery .= " CreatedBy in (select user_id from tblusers where is_deleted=0 and state_code = '".$this->loginData->state_code."' and user_mode= 1)";
		  }
      //print_r($sqlQuery); die();
		...
		$content['subview'] = 'Drill_downreport';
