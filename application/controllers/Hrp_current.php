...
class Hrp_current extends Auth_controller {
	...
	public function index($HRP_Pdf = ' ')
	{
     // start permission 
		...
		$this->load->model('Hrp_current_model');
		$hrp_summary = $this->Hrp_current_model->get_hrp_summary();

		$content['hrp_summary'] = $hrp_summary;

		$content['total_pregnant_women'] = $this->Hrp_current_model->get_total_pregnant_women();
...
/*		$total = 0;
		...
		$content['subview'] = 'hrp_current_report';
